...
<body class="h-100">
    <div class="authincation h-100">
        ...
        @csrf   
         @method('PUT')
           <h3 style="color: #8f0acd;font-weight: bold;text-align: center;font-size: 29px;">Edit category</h3>  
       
                    
            <div class="mb-3 col-md-8">
                       <b><label>Option</label></b><br>
                              <input type="radio" name="option"  value="Course"  @if(@$data->option == "Course") checked @endif required>Course
                                <input type="radio" name="option"  value="Product"  @if(@$data->option == "Product") checked @endif required>Product
                              </div>
                            
                         <div class="mb-3 col-md-8">
                       <b> <label>category</label></b>
                            <input type="text" class="form-control" name="category" value="@if (!empty($data)){{ $data->category }}@else{{ old('category') }}@endif"  required>
                            </div>  
                     </div>  
